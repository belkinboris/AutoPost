...
#
# На SQLite база каждый раз новая (временный файл выше), а на Postgres --
# та же самая. Из-за этого прогон на Postgres со второго раза падал не по
# делу: тесты создают пользователей с фиксированными адресами, и второй
# заход упирался в UNIQUE по email. Выглядело как поломка кода, а было
# мусором от прошлого прогона -- и обесценивало ровно ту проверку, которую
# CLAUDE.md велит делать перед выкладкой (и которую я не сделал перед
# инцидентом 03.08). Чистим схему сами; боевой URL сюда попасть не может,
# он читается только из отдельной PYTEST_DATABASE_URL.
if database.engine.dialect.name != "sqlite":
