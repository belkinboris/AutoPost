...
def token_economics(
    ...
    if billing_rub is not None and billing_tokens:
        ...
        result["billing_reconciliation"] = {
            ...
            "billing_to_recorded_ratio": round(billing_tokens / total, 3) if total else None,
            "comment": (
                "billing_to_recorded_ratio -- отношение того, что РЕАЛЬНО выставил "
                "провайдер, к тому, что записано в Post.tokens_used. "
                "Больше 1 -- часть расхода идёт мимо постов (классификация тем, анализ "
                "стиля, консультант), себестоимость выше записанной. "
                "МЕНЬШЕ 1 -- в постах записано больше, чем списал провайдер: значит "
                "tokens_used это внутренняя единица учёта, а не провайдерский токен, и "
                "себестоимость НИЖЕ записанной во столько же раз. "
                "Себестоимость домножается на этот коэффициент в любом случае."
            ),
        }
    ...
    # Себестоимость домножаем на коэффициент из сверки с биллингом. Он может
    # быть как больше 1 (часть расхода идёт мимо постов: классификация тем,
    # анализ стиля, консультант), так и МЕНЬШЕ 1 -- если Post.tokens_used это
    # внутренняя единица учёта, а не провайдерский токен. Второе не гипотеза:
    # на реальных данных отношение оказалось около 0.17, то есть в постах
    # записано почти вшестеро больше, чем выставил провайдер. Без сверки
    # коэффициент = 1, и цифра может врать в любую сторону -- это помечено в
    # overhead_warning.
    overhead = 1.0
    recon = result.get("billing_reconciliation")
    if recon and recon.get("billing_to_recorded_ratio"):
        overhead = recon["billing_to_recorded_ratio"]
    ...
    if overhead == 1.0:
        result["overhead_warning"] = (
            "Сверка с биллингом не проводилась (не переданы billing_rub и billing_tokens), "
            "поэтому себестоимость посчитана по Post.tokens_used как если бы это были "
            "провайдерские токены. Это может врать в обе стороны и на реальных данных "
            "врало в 5-6 раз В БОЛЬШУЮ сторону. Для настоящей цифры передайте данные биллинга."
        )
