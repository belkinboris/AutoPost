...
def _is_duplicate(a: str, b: str) -> bool:
    ...
    разные слепые зоны, и вместе они закрывают оба типа близнецов
    (новостной пересказ и биографический сторителлинг).
    """
    return _similarity(a, b) >= DUPLICATE_THRESHOLD or _overlap(a, b) >= DUPLICATE_OVERLAP_THRESHOLD


# Порог Жаккара. Откалиброван на паре близнецов-новостей с прода (два поста
# про тайное крещение Путина, созданные в одну минуту) и на парах «разные
# события внутри одной темы канала».
#
...
# -- именно на это и была жалоба (дважды: 28.07 и 02.08).
DUPLICATE_THRESHOLD = float(os.getenv("DUPLICATE_THRESHOLD", "0.10"))

# Порог для коэффициента перекрытия -- своя шкала, он всегда выше Жаккара.
#
# Замеры на реальных и реконструированных парах (перекрытие):
#   близнецы, калибровочная пара            0.63
#   близнецы разной длины (короткий+длинный) 0.26   <- Жаккар здесь всего 0.071
#   худшая пара «разные события»            0.13
# 0.22 ловит асимметричный случай и оставляет почти двукратный запас до
# ложных срабатываний. Асимметрия по длине -- реальный сценарий: у канала
# меняется post_length, формат story даёт длинные посты, news -- короткие,
# и Жаккар такие пары штрафует именно за разницу объёма, а не содержания.
DUPLICATE_OVERLAP_THRESHOLD = float(os.getenv("DUPLICATE_OVERLAP_THRESHOLD", "0.22"))
...
async def _generate_for_channel_impl(channel_id: int, topic: str = "", force_pending: bool = False,
    ...
    if dup:
        ...
        try:
            ...
            # Раньше здесь стояло «оставляю исходный пост» -- то есть
            # ЗАВЕДОМЫЙ дубль (совпадение уже подтверждено выше) уезжал в
            # очередь из-за временной ошибки провайдера на второй попытке.
            # Соседняя ветка при том же исходе пост не создаёт; делаем так же.
            logger.warning(
                f"Канал {channel_id}: перегенерация не удалась ({e}) -- "
                f"пост не создаю, это подтверждённый дубль"
            )
            return {
                "ok": False,
                "message": "Свежих событий по теме пока нет — попробуем позже.",
