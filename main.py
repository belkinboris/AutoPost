...
def _channel_dict(s, ch: Channel) -> dict:
    ...
    # Сколько минут разброса реально применяется. Настройку человек задаёт
    # одну, но она зажимается половиной интервала и длиной окна публикации
    # (см. tasks._jitter_limit) -- и если экран покажет выбранное, а система
    # применит другое, это ровно то враньё, из-за которого окно публикации
    # полгода «работало» вхолостую. Считаем на сервере, чтобы правило жило в
    # одном месте, а не двумя копиями.
    d["jitter_effective_minutes"] = tasks._jitter_limit(ch)
    # ВСЕ рискованные запросы -- в ОТДЕЛЬНОЙ сессии, а не в переданной `s`.
    #
    # Прод-инцидент 03.08, воспроизведён на живом Postgres. Здесь стоял
    # `s.rollback()` при ошибке -- он спасал от «current transaction is
    # aborted» для следующего канала, но приносил беду страшнее: rollback
    # ПРОТУХАЕТ ВСЕ объекты сессии, а `list_channels` загрузил все каналы
    # пользователя заранее и обходит их по очереди. Для уже протухшего
    # объекта `ch.model_dump()` не подтягивает данные заново -- он читает
    # пустой __dict__ и возвращает None во всех полях. На экране это
    # выглядело так: первый канал целый, а все следующие -- без названия,
    # без хэндла, с плашкой «Канал ещё не подключён». Ровно то, что владелец
    # прислал скриншотом: два реальных подключённых канала «сломались».
    #
    # Данные при этом целы -- врал только ответ API. Отдельная сессия
    # разрывает связь: что бы ни случилось с обогащением карточки одного
    # канала, объекты остальных не трогаются вовсе.
    with session() as s2:
        _enrich_channel_dict(d, s2, ch)
    return d


def _enrich_channel_dict(d: dict, s, ch: Channel) -> None:
    try:
